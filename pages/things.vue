  ...
  <div>
    ...
    <main class="list-wrapper">
      <div class="p-6">
        ...
        <h1>Things to do</h1>
        <p class="page-description">Browse local attractions in your area</p>
      </div>
      <div class="list-container p-4">
        <ul class="list">
          <li v-for="link in links" :key="link.name">
            <nuxt-link :to="link.href" class="flex items-start w-full gap-4">
              <img alt="deliver-logo" :src="link.icon" />
              <div class="flex-1">
                <p>{{ link.name }}</p>
                <span class="text-gray-600">{{ link.promo }}</span>
                <p class="text-secondary">{{ link.distance }}</p>
              </div>
              <span class="iconEccobell-carret"></span>
            </nuxt-link>
          </li>
        </ul>
      </div>
...
<script setup lang="ts">
  ...
  const links = ref([
    ...
    },
    {
      ...
      promo: "Free",
      icon: "/img/old-spitadel.png",
      distance: "0.4km",
      href: "/",
    },
    {
      name: "London Street Art Tour",
      promo: "30% off",
      icon: "/img/london-street.png",
      distance: "0.5km",
      href: "/",
    },
    {
